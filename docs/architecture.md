...
**Joining a workspace is always the invitee's own act.** An invitation is never applied automatically. Earlier builds auto-accepted every pending invite at login, reasoning that the manager had already authorized it — but that conflates two consents: manager authorization means the *workspace* agrees to the user joining, and says nothing about whether the *user* agrees to join it. In practice anyone knowing an address could place that person in their workspace, across several orgs in one sign-in, with no way to refuse. `GET /orgs/my-invites` now only *lists* invitations, and it omits any addressed to a workspace the caller already belongs to (a pending invite outlives joining by another route, and would otherwise read as an offer to join the workspace you are already in).

Both surfaces that render it use `components/PendingInvites.vue`: `/welcome` for accounts with no workspace yet, and the app shell — beside the workspace switcher, since the switcher lists the workspaces you are *in* — for accounts that already have one. Without that second surface an invitation to a *different* workspace is reachable only through its emailed link. Declining is confirmed on both, with the same copy: it is terminal, and re-entry needs a manager to issue a fresh invitation. The shell loads the list on mount, so the store deliberately skips that fetch at bootstrap for accounts that already have a membership rather than requesting it twice.
- Every domain doc (`clients`/`projects`/`subGroups`/`tasks`) carries an immutable `orgId`; every query in the `data` store (`stores/data/`, one slice per collection) filters on it via the shared `requireOrgId()`; `versions`/`notes` derive access from their parent task.

Client-side, `stores/auth.ts` loads memberships via a collection-group query, keeps a persisted `activeOrgId`, and live-subscribes the active member/org/usage docs (all detached on logout/switch; the data store fully resets on any org change — the RouterView is keyed on `activeOrgId` so pages remount). Two helpers hold that discipline, and changes to org scoping belong in them rather than alongside them: `liveDoc()` owns the lifecycle of all three subscriptions (attach on activation, detach on logout, clear the ref *before* reattaching so a previous workspace's role/plan/counters are never readable during the gap, drop the value on permission loss), and `activateOrg()` is the single path that changes which workspace the app is scoped to (set `activeOrgId`, persist it per account, reattach the three docs). Zero memberships isn't an error: the router funnels to `/welcome`, which leads with any invitations addressed to the account (accept or decline) and offers workspace creation as the alternative, while `/invite/:orgId/:inviteId` remains reachable as a direct link. Accounts that already have a workspace see the same invitations in the app shell instead (above).
