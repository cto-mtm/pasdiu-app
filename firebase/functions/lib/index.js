...
import { logger as logger6 } from "firebase-functions/v2";
...
import { logger as logger4 } from "firebase-functions/v2";
...
// src/helpers/apiErrors.ts
import { logger } from "firebase-functions/v2";
var MANAGER_ROLES = ["admin", "pm"];
...
// src/helpers/stripeHandlers.ts
import { FieldValue, Timestamp, getFirestore } from "firebase-admin/firestore";
import { logger as logger3 } from "firebase-functions/v2";

// src/helpers/stripe.ts
import { logger as logger2 } from "firebase-functions/v2";

// ../../shared/dist/plans.js
var PLAN_LIMITS = {
  free: { seatLimit: 2, clientLimit: 3, taskLimit: 500 },
  studio: { seatLimit: 15, clientLimit: 25, taskLimit: 1e4 },
  agency: { seatLimit: 50, clientLimit: -1, taskLimit: -1 }
};
var FREE_LIMITS = PLAN_LIMITS.free;
var PAID_PLANS = ["studio", "agency"];
var DISPLAY_PRICES = {
...
var RoleSchema = external_exports.enum(["admin", "pm", "contractor", "client"]);
var PlanSchema = external_exports.enum(["free", "studio", "agency"]);
...
async function priceIdFor(plan, interval) {
...
async function planForPriceId(priceId) {
  ...
  for (const plan of PAID_PLANS) {
    if (process.env[PRICE_ENV_KEYS[plan].month] === priceId || process.env[PRICE_ENV_KEYS[plan].year] === priceId) {
      return plan;
    }
  }
  if (!dynamicPriceCache || Date.now() - dynamicPriceCacheTime >= CACHE_TTL_MS) {
  ...
  if (dynamicPriceCache) {
    for (const plan of PAID_PLANS) {
      if (dynamicPriceCache[`${plan}_month`] === priceId || dynamicPriceCache[`${plan}_year`] === priceId) {
        return plan;
      }
    }
  }
  try {
    ...
    logger2.warn("planForPriceId lookup failed", { priceId, err });
  }
  return null;
}
  ...
  try {
    ...
    for (const price of prices.data) {
      ...
      if (typeof prod === "object" && prod !== null) {
        const name = ("name" in prod ? prod.name : "").toLowerCase();
        const metaPlan = ("metadata" in prod && prod.metadata?.plan ? prod.metadata.plan : "").toLowerCase();
        const planKey = metaPlan === "agency" || name.includes("agency") ? "agency" : metaPlan === "studio" || name.includes("studio") ? "studio" : null;
        ...
        const intervalKey = isYearly ? "year" : isMonthly ? "month" : null;
    ...
    logger2.warn("refreshPriceCache failed", err);
    dynamicPriceCache = {};
  }
}
var client = null;
...
// src/helpers/stripeHandlers.ts
var PAID_PLAN_IDS = ["studio", "agency"];
...
async function applyBillingUpdate(db, event, orgRef, updates) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (marker.exists) {
      logger3.info("stripe webhook: event already processed", { id: event.id, type: event.type });
      return;
    }
    tx.set(markerRef, {
      type: event.type,
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (!orgId || !plan || !PAID_PLAN_IDS.includes(plan) || !subscriptionId) {
        logger3.error("stripe webhook: checkout.session.completed missing metadata", {
          id: event.id,
          orgId,
          plan,
          subscriptionId
      ...
      if (end) updates.currentPeriodEnd = Timestamp.fromMillis(end * 1e3);
    ...
    case "customer.subscription.updated": {
      ...
      if (!orgRef) {
        logger3.error("stripe webhook: no org for subscription", { id: event.id, subscription: sub.id });
        return;
      }
      ...
      else logger3.warn("stripe webhook: unknown price id on subscription", { priceId, subscription: sub.id });
      if (end) updates.currentPeriodEnd = Timestamp.fromMillis(end * 1e3);
    ...
    case "invoice.payment_failed": {
      ...
      if (!customerId) {
        logger3.error("stripe webhook: invoice without customer", { id: event.id });
        return;
      }
      const q = await db.collection("orgs").where("stripeCustomerId", "==", customerId).limit(1).get();
      if (q.empty) {
        logger3.error("stripe webhook: no org for customer", { id: event.id, customer: customerId });
        return;
      }
    ...
    default:
      logger3.info("stripe webhook: unhandled event type", { id: event.id, type: event.type });
  }
}
async function syncSeatQuantity(db, orgId) {
  try {
    ...
    logger3.info("stripe seat sync", { orgId, seats });
  } catch (err) {
    logger3.warn("stripe seat sync failed (ignored)", { orgId, err });
  }
}

// src/routes/billing.ts
var BILLING_INTERVALS = ["month", "year"];
var billingWebhookHandler = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger4.warn("stripe webhook: signature verification failed", { err });
    throw new ApiError(400, "invalid_signature");
  }
  try {
    await handleStripeEvent(event);
  } catch (err) {
    logger4.error("stripe webhook: handler failed (500 \u2192 Stripe retries)", { id: event.id, type: event.type, err });
    throw new ApiError(500, "handler_failed");
  }
  res.json({ received: true });
});
var billingRouter = express2.Router();
...
import { FieldValue as FieldValue2, getFirestore as getFirestore4 } from "firebase-admin/firestore";

// src/helpers/reconcile.ts
import { getFirestore as getFirestore3 } from "firebase-admin/firestore";
import { logger as logger5 } from "firebase-functions/v2";
...
async function reconcileOrg(orgId) {
  ...
  const healed = before.seats !== after.seats || before.activeClients !== after.activeClients || before.activeTasks !== after.activeTasks;
  if (healed) {
    await usageRef.set(after, { merge: true });
    logger5.info("usage reconciled", { orgId, before, after });
  }
...
// src/routes/orgs.ts
var orgsRouter = express3.Router();
...
orgsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    batch.set(
      ...
      { displayName, email, createdAt: FieldValue2.serverTimestamp() },
      { merge: true }
    );
    batch.set(orgRef.collection("members").doc(user.uid), {
      ...
      email,
      role: "admin",
      joinedAt: FieldValue2.serverTimestamp()
    });
...
orgsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const seats = usageSnap.get("seats");
      if (typeof seatLimit === "number" && seatLimit !== -1 && typeof seats === "number" && seats >= seatLimit) {
      ...
      tx.update(usageRef, { seats: FieldValue2.increment(1) });
...
// src/api.ts
var stripeSecretKey = defineSecret("STRIPE_SECRET_KEY");
var stripeWebhookSecret = defineSecret("STRIPE_WEBHOOK_SECRET");
var VALID_ROUTES = [
  "GET /health",
  "POST /orgs",
  "PATCH /orgs/:orgId",
...
app.use((req, res, next) => {
  ...
  logger6.info("api request", { method: req.method, path: req.path });
  next();
});
...
// src/triggers/reconcileUsage.ts
import { logger as logger7 } from "firebase-functions/v2";
import { onSchedule } from "firebase-functions/v2/scheduler";
var reconcileUsage = onSchedule(
  { schedule: "every 24 hours", region: "us-east4" },
  async () => {
    const summary = await reconcileAllOrgs();
    logger7.info("usage reconciliation", summary);
  }
);
...
var es = {
  ...
  footer: "Esta invitaci\xF3n se envi\xF3 desde el espacio de trabajo {orgName} en Pasdiu.",
  roles: { admin: "Administrador", pm: "Gestor de proyecto", contractor: "Editor", client: "Cliente" }
};
var DICTIONARIES = { en, es };
...
async function sendInviteEmailFor(db, orgId, inviteId, invite) {
  ...
  const rendered = renderInviteEmail({
    ...
    inviteUrl,
    locale: typeof invite.locale === "string" ? invite.locale : void 0
  });
...
var onInviteCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    const queued = await sendInviteEmailFor(getFirestore5(), orgId, inviteId, event.data.data());
    if (queued) logger8.info("invite email queued", { orgId, inviteId });
  }
);
